...
report_species = function(message_list, species_validation, species_row) {
  if(species_validation$aggregates$number > 0) { # Aggregates
    ...
    for(col in species_validation$aggregates$col_indexes)
      message_list = add(message_list, new("Message", level = "WARN", source = "Data", row = species_row, column = col, text = paste0("Aggregated species codes reported in column ", col)))
  }

...
setMethod("form_type", "IOTCForm3BU", function(form) {
  return(c("3-BU", "3BU")) # For backwards compatibility
})
  ...
  if(common_metadata_validation_results$general_information$reporting_year$valid && reporting_month_valid) {
    ...
    if(!is.na(vessel_ID_VRKey)) {
      ...
      query(connection = DB_E_RAV(),
            ...
            last_recorded AS (
            SELECT MAX(timestamp) AS date_last_record
            FROM authorised_records
            WHERE timestamp <= '", reference_date, "' -- This should be the start of the month for which we get 3BU info
            )
            ...
            INNER JOIN last_recorded lr ON (erav.timestamp = lr.date_last_record)
            ORDER BY 1, 3, 2;")
          )
